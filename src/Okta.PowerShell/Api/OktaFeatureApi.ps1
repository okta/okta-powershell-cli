...
function Get-OktaFeature {
    ...
    Process {
        ...
        if (!$PSCmdlet.MyInvocation.BoundParameters.ContainsKey("FeatureId") -or $null -eq $FeatureId) {
...
function Update-OktaFeatureLifecycle {
    ...
    Process {
        ...
        $LocalVarUri = $LocalVarUri.replace('{featureId}', [System.Web.HTTPUtility]::UrlEncode($FeatureId))
        if (!$PSCmdlet.MyInvocation.BoundParameters.ContainsKey("Lifecycle") -or $null -eq $Lifecycle) {
        ...
        if ($PSCmdlet.MyInvocation.BoundParameters.ContainsKey("Mode")) {
